...
{
    public static class IFCGeomUtils_IFC2X3
    {


        public static PointIntList ParseExtruded(IIfcExtrudedAreaSolid solid)
        {
            PointIntList curve;
            ...
            {
                curve = ParseCurve(arb.OuterCurve);
            }
            else
            {
        ...
        {
            double Z = 0;
            if(axis.DirectionRatios.Count == 3)
                Z = axis.DirectionRatios[2];
            return new ArrayDouble(axis.DirectionRatios[0], axis.DirectionRatios[1], Z);
        }

        public static PointIntList ParseCurve(IIfcCurve curve)
        {
            if (curve is IIfcBoundedCurve bounded)
            {
                return ParseBoundedCurve(bounded);
            }
            else
            {
